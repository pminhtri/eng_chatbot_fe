...
const spreadStyle = {borderBottom:"solid #F4F5F6 2px",padding:"0 0 20px 30px"}
const boxStyle = {borderRadius:"15px",backgroundColor:"white",boxShadow:"5px 5px 6px 1px #E8EBEC"}
const DashBoard = () => {
    ...
    return ( 
        <Container sx={{display:"flex", flexDirection:"column",alignItems:"center", padding:"3rem", gap:"20px"}}>
            <Stack direction="row" spacing={4} sx={{alignItems:"baseline"}}>
                <Box sx={{padding: "20px 0", ...boxStyle}}>
                    <Box sx={spreadStyle}>
                        <span style={{fontWeight:"bold", fontSize:"20px"}}>Monitoring activities chart</span>
                    </Box>
                    <BarChartCustom requests={requestsInWeek} onItemClick={showDetailByDate}/>
                </Box>
                <Box sx={{padding: "20px 0", ...boxStyle}}>
                    <Box sx={spreadStyle}>
                        <span style={{fontWeight:"bold", fontSize:"20px"}}>Monitoring activities chart</span>
